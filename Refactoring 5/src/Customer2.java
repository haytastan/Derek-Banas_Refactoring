// ***Replace constructors with factories (using factory design pattern).
// unless we have too many subclasses, we still better not to use java reflection***

public abstract class Customer2 {
	
	private String custRating;
		//	static final int PREMIER = 2;
		//	static final int VALUED = 1;
		//	static final int DEADBEAT = 0;
	
	public String getCustRating(){ return custRating; }
	
	public void setCustRating(String custRating) { this.custRating = custRating; }
	
	public static void main(String[] args){
		
		// This factory will generate specific subclasses of Customer3
		
		CustomerFactory customerFactory  = new CustomerFactory();
		
		// This assigns the methods and fields for the class Premier
		// ***This is replaced when I get rid of the Switch 
		//	Customer3 goodCustomer = customerFactory.getCustomer(Customer2.PREMIER);***
		
//		**we directly pass the class name using java reflection**
		Customer2 goodCustomer = customerFactory.getCustomer("Premier");
//		**Customer2 goodCustomer = new Premier(); yazmak ile ayný**
//		**Customer2 goodCustomer = customerFactory.getCustomer(Customer2.PREMIER); 
//		yazmak yine de çok kötü bir alternatif deðil**
		
		System.out.println("This Customers Rating: " + goodCustomer.getCustRating());
		
		// This assigns the methods and fields for the class Deadbeat
		// This is replaced when I get rid of the Switch 
		// Customer3 badCustomer = customerFactory.getCustomer(Customer2.DEADBEAT);
		
		Customer2 badCustomer = customerFactory.getCustomer("Deadbeat");
				
		System.out.println("This Customers Rating: " + badCustomer.getCustRating());
		
	}

}

class Premier extends Customer2{
	
	Premier(){
		
		setCustRating("Premier Customer");
		
	}
	
}

class Deadbeat extends Customer2{
	
	Deadbeat(){
		
		setCustRating("Deadbeat Customer");
		
	}
	
}

class CustomerFactory{
	
//	// ***This is relatively poor design because this switch needs updated
//	// every time I make a new subclass***
//	public Customer2 getCustomer(int custType){
//		
//		switch (custType){
//		
//		case 2:
//			return new Premier(); // **return ettiði için break kullanýlmaz**
////			**alt:** 
////			Premier premier = new Premier(); (veya, Customer2 premier = new Premier();)
////			return premier; 
//		case 0:
//			return new Deadbeat();
//		default:
//			throw new IllegalArgumentException("Invalid Customer Type");
//		
//		}
//		
//	}
	
	public Customer2 getCustomer(String custName){
		
		try {
			
			// ***forName returns a class object with the String that is
			// passed to it. newInstance() creates an instance of the class***
			Customer2 customer2 = (Customer2) Class.forName(custName).newInstance();
//			**casted from object to customer2**
			
//			***package kullanýlmýþ olsaydý kodun formatlanmasý gerekecekti:***
//			String packageName = this.getClass().getPackage().getName();
//			return (Customer2) Class.forName(packageName + “.” + custName).newInstance();
			
			return customer2;
//			**returns new Premier() and new Deadbeat() in program.**
		}
		
//		***exception triggered if a class that doesnt exist is attempted to be created***
		catch(Exception e){
			
			throw new IllegalArgumentException("Invalid Customer Type");
		
		}
		
	}
	
}
/*
This Customers Rating: Premier Customer
This Customers Rating: Deadbeat Customer
*/